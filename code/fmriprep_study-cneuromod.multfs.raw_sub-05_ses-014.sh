...
#SBATCH --job-name=fmriprep_study-cneuromod.multfs.raw_sub-05_ses-014.job
#SBATCH --output=/lustre04/scratch/bpinsard/cneuromod.multfs.fmriprep/code/fmriprep_study-cneuromod.multfs.raw_sub-05_ses-014.out
#SBATCH --error=/lustre04/scratch/bpinsard/cneuromod.multfs.fmriprep/code/fmriprep_study-cneuromod.multfs.raw_sub-05_ses-014.err
...
flock --verbose /lustre03/project/rrg-pbellec/ria-beluga/alias/cneuromod.multfs.fmriprep/.datalad_lock datalad clone ria+file:///lustre03/project/rrg-pbellec/ria-beluga#~cneuromod.multfs.fmriprep@base $LOCAL_DATASET
cd $LOCAL_DATASET
...
datalad containers-run -m 'fMRIPrep_sub-05/ses-014' -n bids-fmriprep --input sourcedata/templateflow/tpl-MNI152NLin2009cAsym/ --input sourcedata/templateflow/tpl-OASIS30ANTs/ --input sourcedata/templateflow/tpl-fsLR/ --input sourcedata/templateflow/tpl-fsaverage/ --input sourcedata/templateflow/tpl-MNI152NLin6Asym/ --output . --input 'sourcedata/cneuromod.multfs.raw/sub-05/ses-014/fmap/' --input 'sourcedata/cneuromod.multfs.raw/sub-05/ses-014/func/'  --input 'sourcedata/smriprep/sub-05/anat/' --input sourcedata/smriprep/sourcedata/freesurfer/fsaverage/ --input sourcedata/smriprep/sourcedata/freesurfer/sub-05/ -- -w ./workdir --participant-label 05 --anat-derivatives sourcedata/smriprep --fs-subjects-dir sourcedata/smriprep/sourcedata/freesurfer --bids-filter-file code/fmriprep_study-cneuromod.multfs.raw_sub-05_ses-014_bids_filters.json --output-layout bids --ignore slicetiming --use-syn-sdc --output-spaces MNI152NLin2009cAsym T1w:res-iso2mm --cifti-output 91k --notrack --write-graph --skip_bids_validation --omp-nthreads 8 --nprocs 12 --mem_mb 45056 --fs-license-file code/freesurfer.license  sourcedata/cneuromod.multfs.raw ./ participant 
fmriprep_exitcode=$?

flock --verbose /lustre03/project/rrg-pbellec/ria-beluga/alias/cneuromod.multfs.fmriprep/.datalad_lock datalad push -d ./ --to origin
if [ -d sourcedata/freesurfer ] ; then
    flock --verbose /lustre03/project/rrg-pbellec/ria-beluga/alias/cneuromod.multfs.fmriprep/.datalad_lock datalad push -J 4 -d sourcedata/freesurfer --to origin
fi 
if [ -e $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json ] ; then cp $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json /scratch/bpinsard/fmriprep_study-cneuromod.multfs.raw_sub-05_ses-014_resource_monitor.json ; fi 
if [ $fmriprep_exitcode -ne 0 ] ; then cp -R $LOCAL_DATASET /scratch/bpinsard/fmriprep_study-cneuromod.multfs.raw_sub-05_ses-014 ; fi 
